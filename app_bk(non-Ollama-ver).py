...
LOCAL_VISION_MODELS = {
    "LOCAL: Qwen2-VL 7B": {
        "id":      "Qwen/Qwen2-VL-7B-Instruct",
        "desc":    "完全ローカル推論 / 日本語高精度 / MPS(M4)",
        "backend": "local",
        "display": "Qwen2-VL 7B [LOCAL / MPS]",
    },
}
...
_defaults = {
    ...
    "is_live":               False,
    "stream_title":          "",
    "selected_model":        "LOCAL: Qwen2-VL 7B",
...
def load_qwen2vl():
    """
    Qwen2-VL 7B をローカルロード（完全オフライン・MPS対応）。
    初回ロード時は ~10GB のキャッシュを読み込むため 1〜2 分かかります。
    社外ネットワークで download_models.py を実行済みである必要があります。
    """
    if not DETECTION_AVAILABLE:
        return None, None, None, "torch/transformers not installed"
    if not _QWEN2VL_CACHED:
        return None, None, None, "Qwen2-VL 未キャッシュ。社外で download_models.py を実行してください"
    try:
        from transformers import Qwen2VLForConditionalGeneration, AutoProcessor
        device = ("mps" if torch.backends.mps.is_available() else
                  "cuda" if torch.cuda.is_available() else "cpu")
        # MPS では float16 が不安定なため bfloat16 を使用
        dtype  = torch.bfloat16 if device in ("mps", "cuda") else torch.float32
        model_id = "Qwen/Qwen2-VL-7B-Instruct"
        processor = AutoProcessor.from_pretrained(
            model_id,
            trust_remote_code=True,
            local_files_only=True,   # 社内ネットワーク遮断対策：キャッシュのみ使用
        )
...
def _local_qwen2vl_analyze(frame_rgb, prompt, resize_pct, max_tokens) -> dict:
    """Qwen2-VL 7B によるローカル推論（MPS高速化版）
    MPS では1トークン約1秒のため max_tokens を 150 に制限し、
    画像も 336x336 に縮小して処理時間を短縮する。
    """
    processor, model, device, err = load_qwen2vl()
    if err:
        return {"ok": False, "text": f"Qwen2-VL ロードエラー: {err}", "latency": 0.0}

    # MPS 高速化: 画像を 336x336 に縮小（Qwen2-VL の最小タイル単位）
    pil_img = Image.fromarray(frame_rgb).resize((336, 336), Image.LANCZOS)
    img_b64 = frame_to_base64(frame_rgb)  # 表示用は元サイズ

    # MPS では長文生成が非常に遅いため 150 トークンに制限
    local_max_tokens = min(max_tokens, 150)
    ...
    try:
        ...
        return {"ok": False, "text": f"Qwen2-VL 推論エラー: {exc}", "latency": time.time() - t0}
...
def vlm_analyze(frame_rgb, prompt, model_name, resize_pct=100, max_tokens=600) -> dict:
    ...
    if cfg["backend"] == "local":
        return _local_qwen2vl_analyze(frame_rgb, prompt, resize_pct, max_tokens)
    if cfg["backend"] == "nim":
